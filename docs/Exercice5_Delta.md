...
1. **Qu'est-ce qu'une synchronisation Delta ?**
   - Au lieu de retourner toutes les données, retourner uniquement les **modifications** depuis la dernière requête (selon une date renseignée en paramètre)
...
3. **Quelles opérations doivent être synchronisées ?**
   ...
   - **Suppressions** : non gérées dans cet exercice (si vous avez du temps devant vous, n'hésitez pas)
